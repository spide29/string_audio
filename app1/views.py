...
def first(request):
    ...
    return render(request, 'first.html',{'options': options})
...
def aop_language(request):
    ...
    # return render(request, 'five.html',{'child_name': child_name, 'avatar_url': avatar_image,'options': options})
...
def red_start(request):
    ...
    if request.method == 'GET':
        ...
        # do some processing on the enrollment_id
...
def msg_api(request):
    if "search" in request.POST:
        ...
        if response.status_code == 200:
            ...
            # print(name)
            request.session['name'] = name
            status = data[0]['status']  # assuming there is only one item in the list
            print(status)
            request.session['status'] = status
            context = {
                'data': data,
...
def language(request):
    ...
    return render(request, 'five.html',{'child_name': child_name, 'avatar_url': avatar_image,'options': options,'eng_level':eng_level})
...
def start_assesment(request,selected_option):
    ...
    # return render(request, 'six.html', {'json_data': json_data})
    return render(request, 'six.html',{'selected_option':selected_option,'selected_level':selected_level,'status':status})
  ...
  if request.method == 'POST':
    try:
            ...
            if fluency_adjustment == '0':
                ...
                return redirect('bl_answer_page')
            # print(response.text)
...
def ml2_answer_page(request):
    ...
    return render(request, "AOP_PRO/ml2_cong.html", context=context)
...
def ml3_answer_page(request):
    ...
    return render(request, "AOP_PRO/ml3_cong.html", context=context)
...
def save_file(request):
    if request.method == 'POST' and request.FILES.get('audio_blob'):
        ...
        with open(os.path.join(media_folder, filename), 'wb') as audio_file:
            ...
            return render(request, 'nine.html', { 'filepath': filepath})


# def para_skip(request):
    ...
    # Format wcpm to have only two decimal places
        ...
        if filepath:
            ...
            if os.path.exists(filepath):
                ...
                filepath = None  
        return render(request, 'nine.html', {'transcript': data_string, 'text':data_string, 'originaltext':val , 'sub_details':index,'del_details':deld, 'audio_url': audio_url, 'no_mistakes': mistake, 'wcpm': wcpm_formatted})
def skip_answer(request):
    # filepath = os.path.join(settings.MEDIA_ROOT, 'skip_para.wav')
    ...
    last_index = len(my_list) 
    return render(request, 'nine.html', {'originaltext': val, 'wcpm': None, 'no_mistakes': last_index})
    ...
    #     mistake = response.json().get('no_mistakes')
    #     fluency = response.json().get('wcpm') 
    #     index = response.json().get('no_sub') 
    #     deld = response.json().get('del_details')

    # # Format wcpm to have only two decimal places
    #     wcpm_formatted = '{:.2f}'.format(float(fluency)) if fluency else None   
    #     filepath = 'media/skip_para.wav'
    #     print("filepath",filepath)
    #     audio_url = None
    #     if filepath:
    #         # Check if the file exists
    ...
    #         else:
    #             filepath = None  
    # return render(request, 'nine.html', { 'originaltext':val , 'wcpm': None})
...
def next_page_letter(request):
    if request.method == 'POST':
        ...
        # return HttpResponse(fluency)
        if total_mis <= '1':
            return redirect("word_ltr")
        else:
            return redirect("word_beg")
...
def next_para(request):
    ...
    if val:
        print("the val",val)
    return render(request, "eight_one.html", {"recording": True,"val":val })
...
def story_recording(request):
    ...
    return render(request, "eleven.html", context)
...
def save_story(request):
    if request.method == 'POST' and request.FILES.get('audio_blob'):
        ...
        with open(os.path.join(media_folder, filename), 'wb') as audio_file:
            ...
            print(filepath)
            request.session['filepath'] = filepath
            return render(request, 'twelve.html', { 'audio_url': filepath})
...
def story_answer(request):
    ...
    if response.status_code == 200: 
        ...
        return render(request, 'twelve.html', {'transcript': data_string, 'text':data_string , 'originaltext':val ,'sub_details':index,'del_details':deld,'audio_url': audio_url, 'no_mistakes': mistake, 'wcpm': wcpm_formatted})
...
def skip_story_answer(request):
    ...
    last_index = len(my_list) 
    return render(request, 'twelve.html', {'originaltext': val, 'wcpm': None, 'no_mistakes': last_index})
    # url = 'http://3.7.133.80:8000/gettranscript/'
    # files = [('audio', (filepath, open(filepath, 'rb'), 'audio/wav'))]
    # payload = {'language': selected_option,'question':val}
    # headers = {}
    ...
    #     return render(request, 'twelve.html', {'transcript': data_string, 'text':data_string , 'originaltext':val , 'sub_details':index,'del_details':deld, 'no_mistakes': mistake, 'wcpm': wcpm_formatted})
...
def next_story(request):
    ...
    if val:
        print("the val",val)
    return render(request, "eleven_one.html", {"recording": True,"val":val })
...
def word_recording(request):
    ...
    return render(request, "fifteen_one.html", context)
...
def save_word(request):
    ...
    if request.method == 'POST' and request.FILES.get('audio_blob'):
        ...
        request.session['filepath'] = filepath
        return render(request, 'fifteen_one.html', { 'audio_url': filepath})
...
# def skip_word_answer(request):
   
#     d_audio_files = request.session.setdefault('d_audio_files', [])
#     print("d_audio_files  word_answer",d_audio_files)


#     filepath = 'media/skip_word.wav'
#     d_audio_files.append(filepath)
#     print("d_audio_files  word_answer",d_audio_files)
...
def letter_recording(request):
    ...
    return render(request, "fifteen_two.html", context)
...
def letter_recording_next(request):
    ...
    Letter, data_id = get_random_letter(request)
    context = {"val": Letter, "recording": True, "data_id": data_id}
    return render(request, "fifteen_two_next.html", context)
...
def save_letter(request):
    ...
    if request.method == 'POST' and request.FILES.get('audio_blob'):
        ...
        request.session['filepath'] = filepath
        return render(request, 'fifteen_two.html', { 'audio_url': filepath})
